...
| `/dev-workflow:review-gate [criteria]` | Run the gate on the current change: deterministic `./.claude/verify`, then a fresh-context Sonnet-class reviewer (Sonnet on Claude / Sol on Codex); escalate to specialists only for high-risk diffs. |
